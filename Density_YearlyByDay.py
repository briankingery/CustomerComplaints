...
# First entry date is 12/4/96
# 
# Change the date range to control how long the script runs at one time. This test is going to test from 19990101 - ErrorOut

    day         = 27                        # for starting day   - Range of 1-28 (Not paying attention to 29, 30, or 31 for differing months)
    month       = 11                        # for starting month - Range of 0-12
    year        = 2012                      # for starting year
    ...
    while year < 2017:                      # change to a more future year as time passes if needed to be ran again
        ...
        # Kernel Density
        inFeatures = "Density_"+CURRENTYEAR+CURRENTMONTH+DAY+"_"+NEXTYEAR+CURRENTMONTH+DAY
        ...
        # Save the output 
        outKernelDensity.save("R:/Divisions/Distribution/Shared/TechnicalScvs/ProjectFiles/EngProjects/Customer_Complaints/Density_YearlyByDay.gdb/Density_"+CURRENTYEAR+CURRENTMONTH+DAY+"_"+NEXTYEAR+CURRENTMONTH+DAY)

        #print "Completed Raster... " + name

        arcpy.Delete_management("Density_"+CURRENTYEAR+CURRENTMONTH+DAY+"_"+NEXTYEAR+CURRENTMONTH+DAY)
        ...
        for df in arcpy.mapping.ListDataFrames(mxd):
            ...
            for raster in arcpy.ListRasters("D*"):              # List all rasters that start with a 'D'
                ...
                for target in targetGroupLayer:
                    ...
                    print "Deleted Raster and Generated Jpeg... " + raster
